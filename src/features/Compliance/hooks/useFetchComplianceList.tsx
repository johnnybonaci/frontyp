import { useEffect, useState } from 'react'
import usePaginatedFetch from 'src/hooks/usePaginatedFetch'
import config from 'src/config'
import { useTranslation } from 'react-i18next'
import { useSnackbar } from 'notistack'
import { type RequestError } from 'src/hooks/useFetch.ts'
import { type Paginator } from 'src/types/paginator'
import { type Filters } from 'src/types/filter'
import { type Sorter } from 'src/types/sorter'
import { type ComplianceItem, type ComplianceItemFromApi } from 'features/Compliance/types'
import { complianceItemFromApi } from 'features/Compliance/transformers'

interface UseFetchComplianceItemsResponse {
  complianceItems: ComplianceItem[] | null
  paginator: Paginator
  loading: boolean
  error: RequestError
  refresh: () => void
  sorter?: Sorter
  setSorter: (fieldName: string, order: 'asc' | 'desc' | undefined) => void
}

const useFetchComplianceList = ({
  filters,
}: {
  filters: Filters
}): UseFetchComplianceItemsResponse => {
  const { t } = useTranslation()
  const { closeSnackbar, enqueueSnackbar } = useSnackbar()
  const [complianceItems, setComplianceItems] = useState<ComplianceItem[] | null>(null)

  const { retry, response, paginator, loading, error, sorter, setSorter } = usePaginatedFetch({
    url: `${config.api.baseUrl}/api/data/leads`,
    filters,
    options: { updatePath: true },
  })

  useEffect(() => {
    if (!response) return

    const complianceItems = response.data.map((role: ComplianceItemFromApi) =>
      complianceItemFromApi(role)
    )

    setComplianceItems(complianceItems)
  }, [response?.data, t])

  useEffect(() => {
    if (!error) return

    enqueueSnackbar(error.message, {
      preventDuplicate: true,
      variant: 'error',
      autoHideDuration: 2000,
      action: (
        <div
          onClick={() => {
            retry()
            closeSnackbar()
          }}
        />
      ),
    })
  }, [error, t])

  return {
    complianceItems,
    paginator,
    loading,
    error,
    refresh: retry,
    sorter,
    setSorter,
  }
}

export default useFetchComplianceList
